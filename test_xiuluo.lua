...
function sleep_run(n)
	...
	repeat 
		t1 = os.clock()
	until((t1-t0) >= (n + gc_software_conf.delay_time/1000))
end
...
-------------move to boss----------------
function confirm_title_move_to_right(drv, cap, n, neg)
	local ret
	local ret1
	local count = 0
	repeat
...
function confirm_title_move_to_left(drv, cap, n, neg)
	local ret
	local ret1
	local count = 0
	repeat
...
function account_proc(conf,user)
	...
	then
		...
		--参数校验
		if(gc_role < 1 or gc_role > 20) then gc_role = 1 end
...
function init_task_to_run()
	...
	win:delete()
	if(check_md5(pn, "36bbdd29a0a3e058426e5efefc424762"))
	then
		account_loop()
